...
import house_Owner.HouseOwnerDB;
...
public class Customer_Login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		CustomerLoginDB cldb = new CustomerLoginDB();
		
		try {
			cldb.Login(username, passw);
			
			if (submitType.equals("customer_login") && passw != null)
			{
				session.setAttribute("uname", username);
				out.println("<html><body><h2> Login Successful!! </h2></body></html>");
				out.println("<a href = 'Customer.jsp'>Click here to visit homepage!</a>");
